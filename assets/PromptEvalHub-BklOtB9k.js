import{r as A,j as e,n as k,L as v}from"./index-0M4frnZE.js";import{S as j}from"./SEOHead-B4vd-YJT.js";import{M as N,r as O}from"./index-y03sU2Ev.js";import{r as D}from"./index-CiXkHzz4.js";/* empty css                 */const q=/←|highlight|주목|중요|important/i;function C(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const M=[{type:"comment",re:/#.*$/gm},{type:"string",re:/("""[\s\S]*?"""|'''[\s\S]*?'''|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')/g},{type:"decorator",re:/@[\w.]+/g},{type:"keyword",re:/\b(import|from|as|def|class|return|if|elif|else|for|while|try|except|finally|with|raise|yield|async|await|pass|break|continue|and|or|not|in|is|lambda|global|nonlocal|del|assert)\b/g},{type:"builtin",re:/\b(True|False|None|self|cls|print|len|range|int|str|float|list|dict|set|tuple|type|isinstance|enumerate|zip|map|filter|super|open|input|sorted|reversed|any|all|min|max|sum|abs|round|format|hasattr|getattr|setattr)\b/g},{type:"function",re:/\b([a-zA-Z_]\w*)\s*(?=\()/g},{type:"number",re:/\b\d+(\.\d+)?\b/g},{type:"operator",re:/([+\-*/%=<>!&|^~:]+|->|\.\.\.)/g}],G=[{type:"comment",re:/(\/\/.*$|\/\*[\s\S]*?\*\/)/gm},{type:"string",re:/(`(?:\\.|[^`\\])*`|"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*')/g},{type:"keyword",re:/\b(import|from|export|default|const|let|var|function|return|if|else|for|while|do|switch|case|break|continue|class|extends|new|this|super|try|catch|finally|throw|async|await|yield|typeof|instanceof|void|delete|in|of)\b/g},{type:"builtin",re:/\b(true|false|null|undefined|NaN|Infinity|console|window|document|Promise|Array|Object|String|Number|Boolean|Map|Set|JSON|Math|Date|RegExp|Error|Symbol|BigInt)\b/g},{type:"function",re:/\b([a-zA-Z_$]\w*)\s*(?=\()/g},{type:"number",re:/\b\d+(\.\d+)?\b/g},{type:"operator",re:/([+\-*/%=<>!&|^~?:]+|=>|\.\.\.)/g}],F=[{type:"comment",re:/#.*$/gm},{type:"string",re:/("(?:\\.|[^"\\])*"|'[^']*')/g},{type:"keyword",re:/\b(if|then|else|elif|fi|for|while|do|done|case|esac|in|function|return|exit|export|source|local|readonly|declare|unset|shift|break|continue)\b/g},{type:"builtin",re:/\b(echo|cd|ls|cat|grep|sed|awk|find|mkdir|rm|cp|mv|chmod|chown|curl|wget|pip|npm|npx|node|python|python3|git|docker|sudo|apt|brew|yarn|pnpm|bun|gh)\b/g},{type:"function",re:/\$\{?\w+\}?/g},{type:"number",re:/\b\d+\b/g},{type:"operator",re:/([|&;<>]+|>>|&&|\|\|)/g}],B=[{type:"string",re:/("(?:\\.|[^"\\])*")\s*(?=:)/g},{type:"string",re:/:\s*("(?:\\.|[^"\\])*")/g},{type:"builtin",re:/\b(true|false|null)\b/g},{type:"number",re:/\b-?\d+(\.\d+)?([eE][+-]?\d+)?\b/g}];function L(t){const i=(t||"").toLowerCase();return["python","py"].includes(i)?M:["javascript","js","jsx","typescript","ts","tsx"].includes(i)?G:["bash","sh","shell","zsh","terminal","console"].includes(i)?F:["json","jsonc"].includes(i)?B:[{type:"comment",re:/(\/\/.*$|#.*$|\/\*[\s\S]*?\*\/)/gm},{type:"string",re:/("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`)/g},{type:"keyword",re:/\b(import|from|export|const|let|var|function|def|class|return|if|else|for|while|try|catch|except|with|async|await|new|this|self)\b/g},{type:"builtin",re:/\b(true|false|null|undefined|None|True|False)\b/g},{type:"number",re:/\b\d+(\.\d+)?\b/g}]}function W(t,i){const n=[];for(const o of i){const a=new RegExp(o.re.source,o.re.flags);let p;for(;(p=a.exec(t))!==null;){const d=p[1]!==void 0&&o.type!=="comment"&&o.type!=="keyword"&&o.type!=="builtin"&&o.type!=="decorator"?p[1]:p[0],y=p.index+(p[0].length-d.length);n.push({start:y,end:y+d.length,type:o.type,text:d})}}n.sort((o,a)=>o.start-a.start||a.end-o.end);const l=[];let s=0;for(const o of n)o.start>=s&&(l.push(o),s=o.end);let m="",r=0;for(const o of l)o.start>r&&(m+=C(t.slice(r,o.start))),m+=`<span class="token ${o.type}">${C(o.text)}</span>`,r=o.end;return r<t.length&&(m+=C(t.slice(r))),m}function H(t,i){const n=L(i);return t.split(`
`).map(s=>{const m=q.test(s),r=W(s,n);return m?`<span class="code-line-highlight">${r}</span>`:r}).join(`
`)}function P({code:t,language:i=""}){const[n,l]=A.useState(!1),s=async()=>{try{await navigator.clipboard.writeText(t),l(!0),setTimeout(()=>l(!1),2e3)}catch{const r=document.createElement("textarea");r.value=t,document.body.appendChild(r),r.select(),document.execCommand("copy"),document.body.removeChild(r),l(!0),setTimeout(()=>l(!1),2e3)}},m=H(t,i);return e.jsxs("div",{className:"code-block-wrapper",children:[e.jsxs("div",{className:"code-block-header",children:[e.jsx("span",{className:"code-block-lang",children:i}),e.jsxs("button",{className:`code-copy-btn ${n?"copied":""}`,onClick:s,children:[e.jsx("i",{className:`fa-solid ${n?"fa-check":"fa-copy"}`}),n?"Copied!":"Copy"]})]}),e.jsx("div",{className:"code-block-content",children:e.jsx("pre",{children:e.jsx("code",{dangerouslySetInnerHTML:{__html:m}})})})]})}const z={tip:"fa-lightbulb",warning:"fa-triangle-exclamation",important:"fa-circle-info",danger:"fa-circle-xmark"},U={tip:"Tip",warning:"Warning",important:"Important",danger:"Danger"};function $({type:t="tip",title:i,children:n}){return e.jsxs("div",{className:`tip-box ${t}`,children:[e.jsx("span",{className:"tip-box-icon",children:e.jsx("i",{className:`fa-solid ${z[t]}`})}),e.jsxs("div",{className:"tip-box-content",children:[i&&e.jsx("strong",{children:i}),!i&&e.jsx("strong",{children:U[t]}),e.jsx("div",{children:n})]})]})}const K={code({inline:t,className:i,children:n,...l}){const s=/language-(\w+)/.exec(i||"");return!t&&s?e.jsx(P,{code:String(n).replace(/\n$/,""),language:s[1]}):!t&&!s&&String(n).includes(`
`)?e.jsx(P,{code:String(n).replace(/\n$/,""),language:""}):e.jsx("code",{className:"inline-code",...l,children:n})},table({children:t}){return e.jsx("div",{className:"table-responsive",children:e.jsx("table",{children:t})})},blockquote({children:t}){return e.jsx($,{type:"tip",children:t})},a({href:t,children:i,...n}){return t&&t.startsWith("/")?e.jsx("a",{href:t,...n,children:i}):e.jsx("a",{href:t,target:"_blank",rel:"noopener noreferrer",...n,children:i})}};function Q({seoTitle:t,seoTitleEn:i,seoDescription:n,path:l,dataFiles:s,ctaBanner:m,sidebarFooter:r}){const{language:o}=k(),a=o==="ko",p=[];s.forEach((c,u)=>{c.sections.forEach(h=>{p.push({section:h,fileIndex:u})})});const[d,y]=A.useState(0),g=p[d],I=g?.fileIndex??0,f=c=>{y(c),window.scrollTo({top:0,behavior:"smooth"})},R=s.length>1,b=[];let E=0;return s.forEach(c=>{b.push(E),E+=c.sections.length}),e.jsxs("div",{className:"guide-page",children:[e.jsx(j,{title:a?t:i||t,description:n,path:l}),e.jsxs("div",{className:"guide-layout",children:[e.jsxs("aside",{className:"guide-sidebar",children:[e.jsx("div",{className:"guide-sidebar-title",children:a?"목차":"Contents"}),e.jsx("ul",{className:"guide-nav",children:R?s.map((c,u)=>{const h=u===I,S=b[u];return e.jsxs("li",{className:`guide-nav-group${h?" expanded":""}`,children:[e.jsxs("button",{className:`guide-nav-group-toggle${h?" active":""}`,onClick:()=>f(S),children:[a?c.title:c.titleEn,e.jsx("i",{className:`fa-solid fa-chevron-${h?"down":"right"} group-chevron`})]}),h&&e.jsx("ul",{className:"guide-nav-sub-list",children:c.sections.map((w,T)=>{const x=S+T;return e.jsx("li",{className:"guide-nav-item",children:e.jsx("button",{className:`guide-nav-link guide-nav-sub ${x===d?"active":""}`,onClick:()=>f(x),children:a?w.title:w.titleEn})},x)})})]},c.id)}):p.map((c,u)=>e.jsx("li",{className:"guide-nav-item",children:e.jsx("button",{className:`guide-nav-link ${u===d?"active":""}`,onClick:()=>f(u),children:a?c.section.title:c.section.titleEn})},u))}),r&&e.jsx("div",{className:"guide-sidebar-footer",children:r})]}),e.jsxs("div",{className:"guide-content",children:[e.jsx("div",{className:"guide-content-header",children:e.jsx("h1",{children:a?g.section.title:g.section.titleEn})}),e.jsx("div",{className:"guide-section",children:e.jsx("div",{className:"markdown-body",children:e.jsx(N,{remarkPlugins:[O],rehypePlugins:[D],components:K,children:a?g.section.content:g.section.contentEn})})}),m,e.jsxs("div",{className:"guide-section-nav",children:[e.jsxs("button",{disabled:d===0,onClick:()=>f(d-1),children:[e.jsx("i",{className:"fa-solid fa-chevron-left"})," ",a?"이전":"Previous"]}),e.jsxs("button",{disabled:d===p.length-1,onClick:()=>f(d+1),children:[a?"다음":"Next"," ",e.jsx("i",{className:"fa-solid fa-chevron-right"})]})]})]})]})]})}const Z={id:"prompt-basics",icon:"fa-lightbulb",title:"프롬프트 기초",titleEn:"Prompt Fundamentals",sections:[{title:"프롬프트란 무엇인가",titleEn:"What is a Prompt",content:`AI 모델에게 원하는 결과를 얻기 위해 전달하는 입력 텍스트를 **프롬프트(Prompt)**라고 합니다.
...
> Prompt writing is not about "getting it perfect the first time" but about "iterative improvement."`}]},J={id:"prompt-evaluation",icon:"fa-clipboard-check",title:"평가 기준 & 루브릭",titleEn:"Evaluation Criteria & Rubrics",sections:[{title:"프롬프트 평가 기준",titleEn:"Prompt Evaluation Criteria",content:`프롬프트의 품질을 객관적으로 평가하기 위한 5대 평가 기준입니다.
...
> Prompt writing skill is not innate — it improves with repeated practice.`}]},_={id:"prompt-techniques",icon:"fa-wand-magic-sparkles",title:"프롬프트 기법",titleEn:"Prompt Techniques",sections:[{title:"핵심 프롬프트 기법",titleEn:"Core Prompt Techniques",content:`AI에서 효과적으로 사용되는 핵심 프롬프트 기법들을 소개합니다.
...
> Advanced strategies are most effective when applied after mastering fundamental techniques.`}]},Y={id:"prompt-examples",icon:"fa-flask",title:"실전 예시 & 실습",titleEn:"Examples & Practice",sections:[{title:"GitHub Copilot 프롬프트 예시",titleEn:"GitHub Copilot Prompt Examples",content:`GitHub Copilot에서 실제 사용하는 프롬프트 예시와 평가입니다.
